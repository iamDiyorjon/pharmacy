...
const STATUS_TABS: { key: TabKey; labelKey: string; color: string }[] = [
	...
	{
		key: "completed",
		labelKey: "orderStatus.status.completed",
		color: "#2e7d32",
	},
	{
		key: "rejected",
		labelKey: "orderStatus.status.rejected",
		color: "#c62828",
	},
...
export default function StaffDashboard() {
	...
	useEffect(() => {
		const unsubscribe = subscribeToStaffOrders(() => {
			loadOrders();
		});
		return unsubscribe;
	}, [loadOrders]);
		...
		<div style={styles.page}>
			...
			<div style={styles.tabs}>
				{STATUS_TABS.map(({ key, labelKey, color }) => {
					...
					if (
						count === 0 &&
						["completed", "rejected", "cancelled"].includes(key)
					)
						return null;
					return (
						<button
			...
			{!loading && activeOrders.length === 0 && (
				<p style={styles.emptyText}>{t("orders.empty", "Buyurtmalar yo'q")}</p>
			)}
					...
					<table style={styles.table}>
						<thead>
							<tr>
								...
								{activeTab === "all" && (
									<th style={styles.th}>{t("orderStatus.title", "Holat")}</th>
								)}
