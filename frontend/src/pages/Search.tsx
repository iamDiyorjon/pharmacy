				...
				<div style={modalStyles.body}>
					...
					<div style={modalStyles.rows}>
						...
						{medicine.requires_prescription && (
							<div style={modalStyles.row}>
								<span style={modalStyles.label}>
									{t("search.requiresPrescription")}
								</span>
								<span style={{ ...modalStyles.value, color: "#e65100" }}>
									✓
								</span>
							</div>
						)}
...
function MedicineCard({
	...
	const avail = medicine.availability.find(
		(a) => a.pharmacy_id === selectedPharmacyId,
	);
	const displayPrice =
		avail?.price != null && avail.price > 0 ? avail.price : null;
	...
	return (
		<div
			style={{ ...cardStyles.card, ...(!isAvailable ? { opacity: 0.5 } : {}) }}
		>
			<div style={cardStyles.row}>
				<div style={cardStyles.info}>
					...
					{!isAvailable && (
						<span style={cardStyles.unavailText}>
							{t("search.unavailable")}
						</span>
					)}
					{medicine.requires_prescription && (
						<span style={cardStyles.rxBadge}>
							{t("search.requiresPrescription")}
						</span>
					)}
					{medicine.category && (
	...
	return (
		<div
			style={{
				...popularStyles.card,
				...(inCart
					? { border: "2px solid var(--tg-theme-button-color, #1976d2)" }
		...
		>
			...
			>
				...
				{price != null ? (
					...
					<div style={popularStyles.price}>{t("search.available")}</div>
				)}
			</div>
...
export default function Search() {
	...
	const [detailMedicine, setDetailMedicine] =
		useState<MedicineWithAvailability | null>(null);
	...
	function goToOrder() {
		navigate("/order", {
			state: {
				items: cart.map((c) => {
					const avail = c.medicine.availability.find(
						(a) => a.pharmacy_id === selectedPharmacyId,
					);
					return {
			...
			{pharmacySelected && (
				<div style={styles.pharmacyBar}>
					<div style={styles.pharmacyBarInfo}>
						...
						<span style={styles.pharmacyBarHint}>
							{selectedPharmacy?.address}
						</span>
					</div>
					<button
						style={styles.pharmacyBarChange}
